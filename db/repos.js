function createRepos(adapter) {
  return {
    workspaces: {
      create({ name, slug, plan = 'free', status = 'active' }) {
        return adapter.insert('workspaces', { name, slug, plan, status });
      },
      get(id) {
    ...
    audit: {
      add({ workspace_id = null, deal_id = null, timestamp = null, agent_name, action_type, details = null, version = null }) {
        return adapter.insert('audit_trail', { workspace_id, deal_id, timestamp, agent_name, action_type, details, version });
      },
      list(workspace_id, opts) {
        const o = opts || {};
